...
def main():
    ...
    print(f"--- 🤖 PORTFOLIO ML BACKTEST BAŞLIYOR ---")
    ...
    # Vektörize metrikler (Tek seferde tüm günler için hesaplanır)
                    ...
                    for ticker, row in top_picks.iterrows():
                        ...
                        if lots > 0:
                            ...
                            if total_cost > current_cash:
                                # Kalan küsurat yetmediyse 1 lot azalt
                                lots -= 1
                                cost = lots * buy_price
                                total_cost = cost * (1 + COMMISSION_RATE)
    ...
    excel_file = "portfolio_backtest_results.xlsx"
    ...
    try:
        ...
        print(f"\nİşlem geçmişi '{excel_file.replace('.xlsx', '.csv')}' dosyasına kaydedildi (Excel renkli çıktı için openpyxl kurun).")

    # 6. GÖRSELLEŞTİRME
    plt.style.use('dark_background')
    fig = plt.figure(figsize=(12, 6))
    plt.plot(daily_vals.index, daily_vals.values, color='cyan', lw=2, label="ML Portfolio Strategy")
    plt.axhline(y=START_CAPITAL, color='white', ls='--', alpha=0.3)
    plt.title(f"ML Portfolio Backtest | Maks Hisse: {N_STOCKS} | Stop Loss: {STOP_LOSS*100}%", fontsize=14)
